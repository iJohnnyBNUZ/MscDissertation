...
University of Edinburgh 2019 MSc Dissertation for Paul Anderson
...
For each module (Client, Server, and Common) :

Set src/main/java to sources root.

Set src/test/java to test root.

If applicable: 

Set src/main/resources to resources root.

Set src/test/resources to test resources root.
...
Set SDK to Java 11. Tested using RedHat OpenJDK and IntelliJ.

## Set up JavaFX:

Download OpenJFX, tested using OpenJFX 11 from https://openjfx.io

If JavaFX related runtime errors are encountered, try the following:
...
Windows: --module-path "\path\to\javafx-sdk-11.0.1\lib" --add-modules javafx.controls,javafx.fxml
...
- can be verified with: java --version

### Set up Server

1. Open terminal 

2. cd into TileLand/JAR directory

3. Run: java -jar Server.jar

4. Press 'n' to load new world, press 'y' to load from previous save
...
3. cd into TileLand/JAR directory

4. Run: java -jar Client.jar

5. Select 'New Game' or 'Continue Game'

6. Enter user name and IP address of server
...
Client main class, starts view and mediator.
...
##### initialKeyAction(Scene scene)

Use the Scene to capture the user's action and put it into the queue.
...
Encapsulates data required by multiple classes. Reduces dependencies throughout the program. Stores instance of the world. Stores queue of local events to be sent to server.

---

### Network

Networking components that connect with server. Uses Java Socket API.

---

#### ClientListener

Listens to updates from server and handles events and objects it receives.

---

#### ClientUpdater 

Updates server with events added to local event queue. 
...
Stores information about entities, locations, and items.
...
Coordinate is to record the position of tiles, entities and items. These objects can in the same coordinate.
...
Server main class, launches mediator.
...
##### sendMessage(Object event) : void

Sends object to client.

##### sendWorld() : void

Sends the world to the client it is connected to.
...
Contains classes that load different components of the game. Reads the JSON files stored in the TileLand/GameFiles/Locations and TileLand/GameFiles/Entities directories.

#### LoadWorld

Loads both entities and locations by calling LoadEntity and LoadLocation. 

#### LoadEntity

Loads entities and their items from JSON files. Can load from a file or from a JSON object.

#### LoadLocation

Loads location files and any items or entities they contain.

#### ReadFile

Reads files and returns its string value.
...
Saves world state to JSON files. Everything is currently stored in TileLand/GameFiles/Locations/Locations.json and TileLand/GameFiles/Entities/Entities.json.

#### SaveWorld

Saves both locations and entities using SaveLocation and SaveEntity.

#### SaveLocation

Saves state of a location to JSON file.

#### SaveEntity

Saves state of an entity to a JSON file
